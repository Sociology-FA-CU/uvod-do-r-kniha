...
::: callout-tip
## Aktualizace výchozího vzhledu geomů
...
```{r update-geoms-example}
#| eval: false

update_geom_defaults(geom = "col", new = list(fill = "#008080"))
```

Obdobně je možné upravovat nastavení všech geomů:

```{r update-geoms-example2}
#| eval: false

update_geom_defaults(geom = "point", new = list(fill = "#008080", size = 12))
update_geom_defaults(geom = "text", new = list(color = "grey", size = 12))
```
:::
...
Jakmile jsou oba balíčky nainstalované, restartujeme Rko. Poté můžeme použít funkci `system_fonts()` k zobrazení seznamu všech fontů, které máme na našem počítači k dispozici. Primární je sloupeček `family`, obsahující název fontu. Ten můžeme využít k nastavení fontu v našich grafech. Například:
